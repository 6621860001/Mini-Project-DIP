...
fastify.post('/bookings', async (request, reply) => {
    ...
    try {
        // ตรวจสอบว่ามีการจองในช่วงเวลาเดียวกันหรือไม่
        const [existingBookings] = await connection.execute(
            'SELECT * FROM Booking WHERE RoomID = ? AND BookingDate = ? AND ((StartTime <= ? AND EndTime > ?) OR (StartTime < ? AND EndTime >= ?))',
            [roomId, bookingDate, startTime, startTime, endTime, endTime]
        );

        if (existingBookings.length > 0) {
            return reply.status(400).send({ error: 'Room is already booked for the selected time' });
        }
